/* autogenerated by Processing revision 1286 on 2024-05-25 */
...
float previoush = 0,h=0,control_value=0,control_value2=0,maxControlLines=0.1f,maxControlLines2=0.1f,alpha=0,EtaCtCp;
...
if (index < control_lines2.size()) {
  ...
  if (test.t>=maxT/20*20 || testcount==1) {
   ...
   if (test.t>=maxT/20*20 ){
    ...
    output_average1.println(Eta+","+alpha/count);
  ...
  while(flagGP==1){
   ...
   nacaName1 = PApplet.parseFloat(pieces1[4]);nacaName2 = PApplet.parseFloat(pieces1[5]);nacaName3 = PApplet.parseFloat(pieces1[6]);Re = PApplet.parseInt(pieces1[7]);
 ...
 public float findMaxValue(ArrayList<String> lines) {
    if (lines.size() == 0) {
...
class RotatingFoil {
  ...
  RotatingFoil( int resolution, int xLengths, int yLengths, float dtReal,float name1,float name2,float name3, int Re, boolean QUICK) {
   ...
   public float directPitch(float t) {
    float pitchAmp = this.dAoA;
    return pitchAmp+AoF; // position control
  }    
   public float directHeave(float t) {
  float HeaveAmp = this.uAoA;
    return HeaveAmp+m/2.f; // position control
    //return HeaveAmp*omega*cos(omega*t); // vel control
  }
    public void setdAoA(float dAoA) {
    this.dAoA = dAoA;  
 }
