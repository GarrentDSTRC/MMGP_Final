...
if __name__ == '__main__': 
    ...
    if(bool(ret)):
        ...
        exit()
    ret = USB_OpenDevice(DevHandles[1])
    if(bool(ret)):
        print("Open device success!")
    else:
        print("Open device faild!")
    ...
    if(bool(ret)):
        ...
        exit()
    ret = DEV_GetDeviceInfo(DevHandles[1],byref(USB2XXXInfo),byref(USB2XXXFunctionString))
    if(bool(ret)):
        print("USB2XXX device infomation:")
        print("--Firmware Name: %s"%bytes(USB2XXXInfo.FirmwareName).decode('ascii'))
        print("--Firmware Version: v%d.%d.%d"%((USB2XXXInfo.FirmwareVersion>>24)&0xFF,(USB2XXXInfo.FirmwareVersion>>16)&0xFF,USB2XXXInfo.FirmwareVersion&0xFFFF))
    ...
    ret = CAN_Init(DevHandles[1],CAN1,byref(CANConfig))
    if(ret != CAN_SUCCESS):
    ...
    ret = CAN_Filter_Init(DevHandles[1],CAN1,byref(CANFilter))
    if(ret != CAN_SUCCESS):
    ...
    CanNum = CAN_GetMsg(DevHandles[1],CAN1,byref(CanMsgBuffer))
    if CanNum > 0:
    ...
    for i in range(0,5):
        ...
        for j in range(0,CanMsg[i].DataLen):
            CanMsg[i].Data[j] = (i<<4)|j
    SendedNum = CAN_SendMsg(DevHandles[1],CAN1,byref(CanMsg),5)
    if SendedNum >= 0 :
