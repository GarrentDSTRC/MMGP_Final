...
def generate_waveform( X, folder_name,mode="CFD"):
    ...
    i=0
    while os.path.exists(f"Trajectory/{i}.csv"):
        i=i+1
   ...
   # 保存Heave波形到文件
    with open(os.path.join(folder_name, "control.txt"), "w") as f:
        for value in z_uniform2:
                f.write(str(value* c) + "\n")
    # 保存Pitch波形到文件
    with open(f"Trajectory/{i}.csv", "w") as f2:
        if mode == "CFD":
            for value in z_uniform:
                f2.write(str(value) + "\n")
        else:
            lines = []
            z_uniformex = np.resize(z_uniform, 38000)
            for j, value in enumerate(z_uniformex):
                row = ",".join([  # 所有参数转为字符串
                    str(j*0.0001),                # 固定值1
                    str(j),             # 索引1
                    "0",                # 固定值2
