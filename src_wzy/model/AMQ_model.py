...
class EnsembleLinear(nn.Module):
    ...
    def get_decay_loss(self) -> torch.Tensor:
        # decay_loss = self.weight_decay * (0.5 * ((self.weight ** 2).sum().item()))
        decay_loss = torch.tensor(self.weight_decay, device=self.weight.device) * (0.5 * (self.weight ** 2).sum())
        return decay_loss
