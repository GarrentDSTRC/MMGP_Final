...
class DynamicModel(nn.Module):
    def __init__(
        ...
        self.encoder = Transformer(self.encoder_dim,
                                   ...
                                   self.encoder_dim * 4,
                                   dropout=0.1,
                                   att_type=self.attn_type) # TODO:dropout, dim_head
...
class DynamicModel_Light(nn.Module):
    def __init__(
        self,
        config,
        decoder_dim_head = 64
        ...
        if self.spectral:
            self.sa_to_obs_next = SpectralRegressor((self.obs_dim + self.action_dim),
                                                       self.embed_dim * 2,
                                                       self.embed_dim,
                                                       self.obs_dim,
                                                       self.modes,
            ...
            self.sa_to_obs_next = nn.Sequential(
                nn.Linear((self.obs_dim + self.action_dim) * self.n_agent, 256),
                nn.SiLU(),
                nn.Dropout(0.05),
    ...
    def forward(self, obs=None, action=None, train=False):
        ...
        full = torch.concat((obs, action), dim=-1)
        if self.spectral:
            mu = self.sa_to_obs_next(full)
            if train:
                mean = mu
                if re_flag == True:
                    mean = rearrange(mean, '(b t) c d -> b t c d', b=b)
            else:
                mean = mu[:, -1, :]
                if re_flag == True:
                    mean = rearrange(mean, '(b t) d -> b t d', b=b)
        else:
            full = full.view(batch_num, -1)
            mean = self.sa_to_obs_next(full)
            if re_flag == True:
                mean = rearrange(mean, '(b t) d -> b t d', b=b)
