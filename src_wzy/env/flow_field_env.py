...
class foil_env:
    ...
    def step(self, action):
        ...
        info = {"ct": np.average(ct_ls, weights=dt_ls),
                ...
                "next_dt": info['next_dt'],
                "CurrentY": info['CurrentY'],
                "CurrentPhi": info['CurrentPhi']}
    ...
    def parseStep(self, info):
        ...
        if not done:
            ...
            dt = state['dt']
            #reward = 1
            # print(np.abs(state['delta_y']))

            # if shift far from the center, we give a penalty
            if np.abs(state['delta_y']) > 22:
                reward -= (np.abs(state['delta_y'])-22)**2/100

            reward += ct * 5 -cp +1#+ ct #TODO: add dt?
        ...
        return state_ls, reward, done, {'ct': state['ct'], 'eta': state['eta'], 'fx': -16*state['ct'], 'cp': state['cp'], 'dt': state['dt'], 'delta_y': state['CurrentY'], 'delta_theta': state['CurrentPhi'], 'next_dt': state['next_dt'],'CurrentY': state['CurrentY'], 'CurrentPhi': state['CurrentPhi']}
