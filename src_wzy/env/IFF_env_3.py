...
class ServoControlEnv(gym.Env):
    def __init__(self, args):
        ...
        self.ini_mid_values = np.ones([self.n_iff, 3]) * 180.0
        self.mid_values = self.ini_mid_values
        self.lower_values = self.mid_values - 30.0
        self.upper_values = self.mid_values + 30.0
    ...
    def save(self, iter, pth_name, save_full_data=True, save_type='csv'):
        # Add pth_name parameter to identify different model files
        ...
        if save_full_data:
            ...
            if save_type == 'csv':
                # Modify filename to include pth name
                filename = self.csvname.format(f"{pth_name}_{iter}")
                ...
                # Modify obs and act filenames to include pth name
                obsname = self.obs_template.format(f"{pth_name}_{iter}")
                with open(obsname, mode='w', newline='') as csv_file:
                    csv_writer = csv.writer(csv_file)
                    csv_writer.writerows(self.obs_array)

                actname = self.act_template.format(f"{pth_name}_{iter}")
            ...
            full_s_list = self.average_Ct + self.average_Cl + cl_v + cl_rms

            # Modify count file to include pth name
            countfile = self.countfile.format(pth_name)
            with open(countfile, mode='a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow(full_s_list)

        print('Iter:', iter, 'Model:', pth_name)
        np.savetxt(self.countname.format(pth_name), np.array([self.n_iff, iter]))
    ...
    def load_midvalue(self, midvalue):
        ...
        for i in range(self.n_iff):
            self.mid_values[i, 0] = midvalue[i * 3]
            self.mid_values[i, 1] = midvalue[i * 3 + 1]
            self.mid_values[i, 2] = midvalue[i * 3 + 2]
    ...
    def step(self, action, break_t=0.035):
        ...
        if not self.current_sensor_data:
            ...
            # 获取当前和历史时刻的fy值的平均
            if len(cls) > 0:
                current_fy = cls[-1]  # 当前fy
                fy_5 = cls[-5] if len(cls) >= 5 else np.zeros_like(current_fy)  # 5步前的fy
                fy_10 = cls[-10] if len(cls) >= 10 else np.zeros_like(current_fy)  # 10步前的fy
                fy_15 = cls[-15] if len(cls) >= 15 else np.zeros_like(current_fy)  # 15步前的fy
            else:
                current_fy = np.zeros(self.n_iff)
                fy_5 = np.zeros(self.n_iff)
                fy_10 = np.zeros(self.n_iff)
                fy_15 = np.zeros(self.n_iff)

            # 计算这四个值的平均作为新的fy_rms
            self.fy_rms = np.mean(np.array([current_fy, fy_5, fy_10, fy_15]), axis=0)
            ...
            reward, done = self.compute_reward_done(zero_state)
        ...
        reward, done = self.compute_reward_done(state)
    ...
    def compute_reward_done(self, state):
        ...
        # print(penalty)
        reward = 1.2 * Clipped_Fx -0.08 * np.abs(Clipped_Fy-0.7) - 0.3 * (np.abs(self.fy_rms - 0.7) * 1.0) - 0.05 * penalty
        # reward = 0.9 * Clipped_Fy - 0.05 * penalty
