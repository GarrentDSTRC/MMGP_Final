...
class ServoControlEnv(gym.Env):
    def __init__(self, args):
        ...
        self.c_alpha = args.c_alpha
        self.c_beta = args.c_beta
        self.c_gamma = args.c_gamma
        self.c_target = args.c_target
    ...
    def step(self, action, break_t=0.035):
        ...
        if not self.current_sensor_data:
            ...
            self.fy_mean = np.mean(cls, axis=0)
            self.pre_mean = np.mean(cls[:-1], axis=0)
            ...
            reward, cost, done = self.compute_reward_done(zero_state)
            self.obs_array.append(zero_state.flatten().tolist())
            return zero_state, reward, cost, done, {'Fx': Fx, 'Fy': Fy, 'Pre': pre_fys}
        ...
        reward, cost, done = self.compute_reward_done(state)
        ...
        return state, reward, cost, done, {'Fx': Fx, 'Fy': Fy, 'Pre': pre_fys}
    ...
    def compute_reward_done(self, state):
        ...
        # print(penalty)
        reward = self.r_alpha * Clipped_Fx + self.r_beta * Clipped_Fy - self.r_gamma * self.fy_rms - 0.05 * penalty
        mean_err = np.abs(self.fy_mean - self.c_target)
        cost = self.c_alpha * np.abs(Clipped_Fy - self.c_target) + self.c_beta * mean_err \
               + self.c_gamma * (mean_err - np.abs(self.pre_mean - self.c_target))
        # reward = 0.9 * Clipped_Fy - 0.05 * penalty
        return reward * 1.0, cost * 0.2, done
