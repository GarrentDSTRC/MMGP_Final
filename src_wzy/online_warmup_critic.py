...
wandb.init(project="IFF_TPPO_Vwarmup", entity="krhkk", config=paras, name=args.name,
...
for i in tqdm(range(1200)):
    ...
    ct_avg = env.average_Ct[0]  # Using the first one.
    cl_avg = env.average_Cl[0]
    train_cnt += 1
    adjust_cnt += 1
    ...
    if train_cnt >= train_target:
        ...
        wandb.log({"Gt": Gt, "length": epsoide_length, "episode_num": i, "avg_reward": Gt / epsoide_length,
                   "ct": ct_avg, 'cl': cl_avg, "policy_entropy": agent.actor.entropy,
                   "critic_loss": agent.critic_loss})

        print("epoch:", i, "length:", epsoide_length, "G:", Gt, "critic_loss",
              agent.critic_loss, " sigma:", agent.actor.sigma_param)
        print("action:", action)

        if i>1 and agent.critic_loss < min_critic:
            min_critic = agent.critic_loss
            agent.save_critic(run_dir, "best")
            print("save best critic model")
