...
class TPPO_B(object):
    ...
    # input dim [batch * context_len * dim], batch -> [num_envs * time_steps]
        ...
        for iter in range(number):
            ...
            # V-trace Calculation
            v_next = self.actors[id].critic.getValue(next_state).reshape(state.shape[0], state.shape[1])
            td_target = reward + self.gamma * v_next * (1 - done)  # 20*1
            td_error = td_target - self.actors[id].critic.getValue(state).reshape(state.shape[0], state.shape[1])
            ...
            for t in reversed(range(state.shape[0] - 1)):
                v_trace_target[t] = reward[t] + self.gamma * v_next[t + 1] * (1 - done[t]) + self.lambda_ * delta[t + 1]
            ...
            # For each actor, calculate the policy loss using V-trace targets
            old_action_log_prob = []
            for actor in self.actors:
                old_action_log_prob.append(actor.getActionLogProb(state, action))
