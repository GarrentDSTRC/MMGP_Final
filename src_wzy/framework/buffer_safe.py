...
class Buffer:
    def __init__(self, config):
        ...
        self.buffer_dict = {}
        self.property_list = ['states', 'states_next', 'rewards', 'costs', 'dones', 'actions']
        # self.buffer_dict_clear()

        self.offline_buffer_dict = {}
        self.offline_properties = ['states', 'states_next', 'states_actions', 'rewards', 'dones', 'actions', 'fx', 'fy', 'pre_fy_rms']
        self.buffer_dict_clear()
    ...
    def sample(self, batch_size=256, length=30):
        for item in self.property_list:
            ...
            elif 'rewards' in item or 'dones' in item or 'costs' in item:
        ...
        cost = self.buffer_dict['costs']
        done = self.buffer_dict['dones']
        state_next = self.buffer_dict['states_next']
        return state, action, reward, cost, done, state_next
    ...
    def sample_offline(self, batch_size=256, length=1):
        ...
        fy_rms = self.offline_buffer_dict['pre_fy_rms'][ind[:, None], ind2]

        return state, action, sa_pair, reward, done, state_next, fx, fy, fy_rms

    # def rearrange_collected(self, length_list, l=118):
    #     for item in self.offline_properties:
    #         if 'state' in item:
    #             if item == 'states_actions':
    #                 self.offline_buffer_dict[item] = np.array(self.buffer_dict[item]).transpose(1, 0, 2, 3) / 12.0
    #                 print("SA_pairs scaled!")
    #             else:
    #                 self.offline_buffer_dict[item] = np.array(self.buffer_dict[item]).transpose(1, 0, 2, 3)
    #         elif 'rewards' in item or 'dones' in item or 'fx' in item or 'fy' in item:
    #             self.offline_buffer_dict[item] = np.array(self.buffer_dict[item]).transpose(1, 0)
    #             # print(item)
    #         elif item == 'actions':
    #             self.offline_buffer_dict[item] = np.array(self.buffer_dict[item]).transpose(1, 0, 2)
    #
    #     self.size = self.offline_buffer_dict['states'].shape[0]
    #     self.size1 = self.offline_buffer_dict['states'].shape[1]
    #
    #     print('Buffer size: ', self.size, self.size1)
    #
    ...
    def rearrange_collected(self, length_list, l=117):
        SS = []
        SN = []
        RR = []
        DD = []
        ...
        PRES = []
        starting = 0
        toadd = len(length_list)
        for xx in range(toadd):
            for item in self.offline_properties:
                # for arr in self.buffer_dict[item]:

                s = self.buffer_dict[item][starting: starting+l]
                s = np.array(s)
                # print(item, starting + l, s.shape)
                if item == 'states':
                    SS.append(s.transpose(1, 0, 2, 3))
                elif item == 'states_next':
                    SN.append(s.transpose(1, 0, 2, 3))
                elif item == 'rewards':
                    RR.append(s.transpose(1, 0))
                elif item == 'dones':
                    DD.append(s.transpose(1, 0))
                elif item == 'actions':
                    AA.append(s.transpose(1, 0, 2))
                elif item == 'states_actions':
                    SA.append(s.transpose(1, 0, 2, 3))
                elif item == 'fx':
                    FX.append(s.transpose(1, 0))
                elif item == 'fy':
                    FY.append(s.transpose(1, 0))
                elif item == 'pre_fy_rms':
                    PRES.append(s.transpose(1, 0))

            starting += length_list[xx]

        c_states = np.concatenate(SS, axis=0)
        # print('cstates', c_states.shape)
        c_next = np.concatenate(SN, axis=0)
        c_r = np.concatenate(RR, axis=0)
        c_d = np.concatenate(DD, axis=0)
        c_a = np.concatenate(AA, axis=0)
        c_sa = np.concatenate(SA, axis=0)
        c_sa = c_sa / 12.0
        c_x = np.concatenate(FX, axis=0)
        c_y = np.concatenate(FY, axis=0)
        c_pre = np.concatenate(PRES, axis=0)
        ...
        for prop in self.offline_properties:
            if prop == 'states':
                new_data = np.concatenate([self.offline_buffer_dict[prop], c_states], axis=0)
            elif prop == 'states_next':
                new_data = np.concatenate([self.offline_buffer_dict[prop], c_next], axis=0)
            elif prop == 'states_actions':
                new_data = np.concatenate([self.offline_buffer_dict[prop], c_sa], axis=0)
            elif prop == 'rewards':
                new_data = np.concatenate([self.offline_buffer_dict[prop], c_r], axis=0)
            elif prop == 'dones':
                new_data = np.concatenate([self.offline_buffer_dict[prop], c_d], axis=0)
            elif prop == 'actions':
                new_data = np.concatenate([self.offline_buffer_dict[prop], c_a], axis=0)
            elif prop == 'fx':
                new_data = np.concatenate([self.offline_buffer_dict[prop], c_x], axis=0)
            elif prop == 'fy':
                new_data = np.concatenate([self.offline_buffer_dict[prop], c_y], axis=0)
            elif prop == 'pre_fy_rms':
                new_data = np.concatenate([self.offline_buffer_dict[prop], c_pre], axis=0)


            if new_data.shape[0] > max_size:
                new_data = new_data[-max_size:]

            self.offline_buffer_dict[prop] = new_data
        ...
        return state, state_next, reward, done, sa_pair
    ...
    def load_pre_fy_rms(self, fys):
        self.offline_buffer_dict['pre_fy_rms'] = list()
        self.offline_buffer_dict['pre_fy_rms'] = fys
        print("FyRMS pre-computed by gae loaded")
    ...
    def load_all_offlines(self, path, path2, gamma, l=117, clip=None):
        #S, S', SA, A, R, D, Rt
        ...
        for item in self.offline_properties:
            ...
            if 'state' in item:
                ...
                if item == 'states_actions':
                    self.offline_buffer_dict[item] = np.array(s) / 12.0
                    print("SA_pairs scaled!")
                else:
                    print(item, s.shape)
                    self.offline_buffer_dict[item] = np.array(s)
            elif 'rewards' in item or 'dones' in item or 'fx' in item or 'fy' in item:
                self.offline_buffer_dict[item] = np.array(s)
                # print(item)
            elif item == 'actions':
                if clip != None:
                    self.offline_buffer_dict[item] = np.clip(np.array(s), -clip, clip)
    ...
    def add_offlines(self, path, gamma, l=119, clip=None):
        #S, S', SA, A, R, D, Rt
        data = pickle.load(open(path, "rb"))
        offline_properties = ['states', 'states_next', 'states_actions', 'rewards', 'dones', 'actions', 'returns', 'fx',
                              'fy']
        ...
        for item in offline_properties:
            ...
            if 'state' in item:
                ...
                if item == 'states_actions':
                    s1 = np.array(s).transpose(1, 0, 2, 3) / 12.0
                    self.offline_buffer_dict[item] = np.concatenate((self.offline_buffer_dict[item], s1), axis=0)
                    print("SA_pairs scaled!")
                else:
                    s1 = np.array(s).transpose(1, 0, 2, 3)
                    self.offline_buffer_dict[item] = np.concatenate((self.offline_buffer_dict[item], s1), axis=0)
            elif 'rewards' in item or 'dones' in item or 'fx' in item or 'fy' in item:
                s1 = np.array(s).transpose(1, 0)
                self.offline_buffer_dict[item] = np.concatenate((self.offline_buffer_dict[item], s1), axis=0)
                # print(item)
            elif item == 'actions':
                if clip != None:
                    s1 = np.clip(np.array(s), -clip, clip).transpose(1, 0, 2)
                    self.offline_buffer_dict[item] = np.concatenate((self.offline_buffer_dict[item], s1), axis=0)
                    print("All actions clipped to {}, {}".format(-clip, clip))
                else:
                    s1 = np.array(s).transpose(1, 0, 2)
                    self.offline_buffer_dict[item] = np.concatenate((self.offline_buffer_dict[item], s1), axis=0)
        ...
        print('Size added: ', self.size, self.size1)
