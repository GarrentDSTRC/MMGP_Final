...
from model.AMQ_model import EnsembleAMQ
...
class Transition(object):
    def __init__(self, args):
        ...
        self.lower_values, self.upper_values = -(np.pi / 3.0), (np.pi / 3.0)
        ...
        if args.amq == True:
            self.with_std = True
            self.dynamics = EnsembleAMQ(obs_dim=self.state_dim, action_dim=self.action_dim, hidden_dims=args.amq_hidden,
                                        weight_decays=args.amq_weight_decay, device=self.device)
            self.amq_optimizer = torch.optim.Adam(self.dynamics.parameters(), lr=args.d_lr)
        else:
            if self.light == True:
                self.dynamics = DynamicModel_Light(args).to(self.device)
            else:
    ...
    def reset_optimizer(self):
        if self.amq:
            self.amq_optimizer = torch.optim.Adam(self.dynamics.parameters(), lr=self.d_lr)
        else:
    ...
    def compute_reward_done(self, state, action, pre_fys=None):
        if len(state.shape) == 3:
            ...
            Clipped_Fy = self.double_symlog_clip(Fy, 8.0, 16.0)
            Clipped_Mz = self.double_symlog_clip(Mz, 10.0, 20.0)

            # reward = np.zeros(self.n_iff)
            exps = np.exp(self.pe / 5.0) - 1.0  # -e^0
            # count_positive = np.sum(self.pe > 1e-3, axis=-1)
            count_positive = np.array((self.pe > 1e-3))
            penalty = exps
            done_condition = np.array((count_positive > 0))
        ...
        if pre_fys is None:
            self.running_cl.append(Clipped_Fy)
            if len(self.running_cl) > 20:
                self.running_cl.pop(0)
                clss = np.array(self.running_cl)
            elif len(self.running_cl) < 20:
                ll = 20 - len(self.running_cl)

                cl00 = np.array(self.running_cl)
                shape = list(cl00.shape)
                shape[0] = ll
                cl11 = np.zeros(shape)
                clss = np.concatenate((cl11, cl00), axis=0)
            else:
                clss = np.array(self.running_cl)
            # clss = np.array(self.running_cl)
            fy_rms = np.sqrt(np.mean(clss ** 2, axis=0))
            # fy_mean = np.mean(clss, axis=0)
            # fy_4 = (clss[4,:] + clss[9,:] + clss[14,:] + clss[19,:]) / 4.0
        else:
            fy_rms = np.sqrt((Clipped_Fy**2 + pre_fys * 19.0) / 20.0)
            # fy_mean = ((Clipped_Fy + pre_fys * 19.0) / 20.0)
            # fy_4 = (pre_fys + Clipped_Fy) / 4.0

        ####FOIL####
        # reward = self.r_alpha * Clipped_Fx + self.r_beta * Clipped_Fy \
        #          - self.r_gamma * fy_rms - 0.05 * penalty
        
        ####TAIL####
        # reward =  1.2 * Clipped_Fx + 0.3 * Clipped_Fy - 0.05 * penalty
        # reward = 0.25 * Clipped_Fx + 1.5 * Clipped_Fy - 0.05 * penalty
        # reward = 1.5 * Clipped_Fx - 0.25 * theta_z * Clipped_Mz - 0.05 * penalty

        reward = 0.8 * Clipped_Fx + 0.75 * Clipped_Fy - 0.08 * theta_z * Clipped_Mz - 0.05 * penalty
        # reward = 1.2 * Clipped_Fx - 0.08 * np.abs(Clipped_Fy - 0.7) - 0.3 * (np.abs(fy_4 - 0.7) * 1.0)- 0.05 * penalty

        # reward = self.r_alpha * Clipped_Fx - self.r_beta * np.abs(Clipped_Fy - 0.7) \
        #          - self.r_gamma * np.abs(fy_mean - 0.7) - 0.05 * penalty
    ...
    def multiple_step(self, obs, action_list, pre_fys=None):
        s_next_list = []
        r_list = []
        for action in action_list:
            next_pred, next_r, _, _ = self.step(obs, action, pre_fys=pre_fys)
    ...
    def step(
        ...
        if self.amq:
            Tobs = obs[..., -1, :]
            Taction = action[..., -1, :]
            Tin = np.concatenate((Tobs, Taction), axis=-1)
            if len(Tin.shape) == 3:
                old_shape = Tobs.shape
                new_shape = (Tin.shape[0] * Tin.shape[1], Tin.shape[2])
                Tin = Tin.reshape(new_shape)
                re_flag = True
            else:
                re_flag = False
            mean, logvar = self.dynamics(Tin)
        else:
            Tobs = torch.FloatTensor(obs).to(self.device)
            Taction = torch.FloatTensor(action).to(self.device)
            mean, logvar = self.dynamics(Tobs, Taction, train=out_with_context)
    ...
    def train(
        ...
        if self.amq:
            self.scheduler = ExponentialLR(self.amq_optimizer, gamma=0.99)
        else:
            self.scheduler = ExponentialLR(self.dynamics.optimizer, gamma=0.99)

        if self.amq:
            holdout_losses = [1e10 for i in range(self.dynamics.num_ensemble)]
            train_losses = []
            all_holdout_losses = []
        else:
            holdout_loss = 1e10
        ...
        while True:
            ...
            if self.amq:
                indexes = []
                train_loss = self.learn_amq(train_obss, train_acts, train_targets, batch_size, logvar_loss_coef)
                new_holdout_losses = self.validate_amq(holdout_obss, holdout_acts, holdout_targets)
                all_holdout_losses.append(new_holdout_losses)
                train_losses.append(train_loss)

                print('Train losses: ', train_loss)
                print('Val losses: ', new_holdout_losses)
                ...
                if self.n_ensemble > 1:
                    holdout_loss = (np.sort(new_holdout_losses)[:self.dynamics.num_elites]).mean()
        ...
        if self.amq:
            ...
            self.save(save_path)
            self.dynamics.eval()
            holdout_losses = all_holdout_losses
    ...
    def learn(
        ...
        for index in BatchSampler(RandomSampler(range(train_size)), batch_size, False):
            ...
            if self.light == True and self.spectral == False:
                targets_batch = targets_batch[:,:,-1,:].squeeze(2)
    ...
    def learn_amq(
        ...
        self.dynamics.train()
        train_size = input_obss.shape[1]
        losses = []

        for index in BatchSampler(RandomSampler(range(train_size)), batch_size, False):
            inputs_o_batch = input_obss[index][:,:,-1,:]
            inputs_a_batch = input_acts[index][:,:,-1,:]
            targets_batch = targets[index][:,:,-1,:]

            inputs = np.concatenate((inputs_o_batch, inputs_a_batch), axis=-1)
            new_shape = (inputs.shape[0] * inputs.shape[1], inputs.shape[2])
            new_shape_t = (inputs.shape[0] * inputs.shape[1], targets_batch.shape[2])
            final_inputs = inputs.reshape(new_shape)
            targets_batch = targets_batch.reshape(new_shape_t)

            final_inputs = torch.FloatTensor(final_inputs).to(self.dynamics.device)
            targets_batch = torch.FloatTensor(targets_batch).to(self.dynamics.device)

            mean, logvar = self.dynamics(final_inputs)
            inv_var = torch.exp(-logvar)
            ...
            self.amq_optimizer.zero_grad()
            loss.backward()
            self.amq_optimizer.step()

            losses.append(loss.item())
        return np.mean(losses)

    @torch.no_grad()
    def validate_amq(self, input_obss: np.ndarray, input_acts: np.ndarray, targets: np.ndarray, batch_size: int = 256) -> float:
        self.dynamics.eval()

        train_size = input_obss.shape[0]
        bts = int(np.ceil(train_size / batch_size))
        val_loss = [0.0] * self.dynamics.num_ensemble

        for batch_num in range(int(np.ceil(train_size / batch_size))):
            inputs_o_batch = input_obss[batch_num * batch_size:(batch_num + 1) * batch_size][:,:,-1,:]
            inputs_a_batch = input_acts[batch_num * batch_size:(batch_num + 1) * batch_size][:,:,-1,:]
            targets_batch = targets[batch_num * batch_size:(batch_num + 1) * batch_size][:,:,-1,:]
            ...
            mean, _ = self.dynamics(final_inputs)

            loss = ((mean - targets_batch) ** 2).mean(dim=(1, 2))
            l_loss = list(loss.cpu().numpy())
            for i in range(self.dynamics.num_ensemble):
                val_loss[i] += l_loss[i] / bts
