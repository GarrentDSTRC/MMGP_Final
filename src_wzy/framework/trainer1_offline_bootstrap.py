...
class TBPPO(object):
    ...
    def load(self, file):
        ...
        self.actor.reset_optimizer()
        self.old_actor.reset_optimizer()


class ETBPPO_B(object):
    #In this bootstrapping training scheme, we also update critic with collected data
    def __init__(self, args):
        self.n_ensemble = args.n_ensemble
        self.ensemble = []
        # self.critic = TransformerAgent(args)
        self.grad_norm = args.grad_norm_clip
        self.gamma = args.gamma
        self.lamda = args.gae_lambda
        self.clip = args.ppo_clip
        ...
        args.mode = 'actor'
        self.base_actor = TransformerAgent(args).to(self.device)
    ...
    def update_critic(self, state, next_state, reward, done):
        ...
        td_target = reward + self.gamma * self.critic.getValue(next_state).reshape(state.shape[0], state.shape[1]) * (
                    1 - done)  # 20*1
        td_error = td_target - self.critic.getValue(state).reshape(state.shape[0], state.shape[1])
        advantage = compute_advantage(self.gamma, self.lamda, td_error.cpu()).to(self.device)
        # [trick] : advantage normalization
    ...
    def choose_action(self, state, id, train=True):
        ...
        self.state = torch.cat([self.state, state], dim=1)[:, 1:, :]
        if train:
            action, _ = self.ensemble[id].actor.getVecAction(self.state)
        else:
            action, _ = self.ensemble[id].actor.getVecAction(self.state, train=False)
        return action

    def save_baseactor(self, id):
        self.base_actor.load_state_dict(self.ensemble[id].actor.state_dict())

    def change_to_baseactor(self, id):
        self.ensemble[id].actor.load_state_dict(self.base_actor.state_dict())
        self.ensemble[id].old_actor.load_state_dict(self.base_actor.state_dict())

    def online_single_learn(self, id):
        state, action, reward, done, next_state = self.memory.sample(self.batch_size)
        state = torch.FloatTensor(state).to(self.device)
        action = torch.FloatTensor(action).to(self.device)
        reward = torch.FloatTensor(reward).to(self.device)
        ...
        td_lamda_target = advantage + self.critic.getValue(state).reshape(state.shape[0], state.shape[1])
        advantage = ((advantage - advantage.mean()) / (advantage.std() + 1e-5)).detach()
        old_action_log_prob = self.ensemble[id].actor.getActionLogProb(state, action)

        for _ in range(self.epoch):
            # sample new action and new action log prob
            print(_)
            new_action_log_prob = self.ensemble[id].actor.getActionLogProb(state, action, train=True)
            # update actor
            ratio = (new_action_log_prob - old_action_log_prob).exp()
            surprise = ratio * advantage
            clipped_surprise = torch.clamp(ratio, 1 - self.clip, 1 + self.clip) * advantage
            ...
            # update
            self.ensemble[id].actor.optimizer.zero_grad()
            self.critic.optimizer.zero_grad()
            actor_loss.backward()
            critic_loss.backward()
            # trick: clip gradient
            nn.utils.clip_grad_norm_(self.ensemble[id].actor.parameters(), self.grad_norm)
            nn.utils.clip_grad_norm_(self.critic.parameters(), self.grad_norm)
            self.ensemble[id].actor.optimizer.step()
            self.critic.optimizer.step()
