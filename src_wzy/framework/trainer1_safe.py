...
from framework.buffer_safe import Buffer as buffer
from datetime import datetime
from framework.pid_lagrange import PIDLagrangian
...
class TPPO(object):
    def __init__(self, args):
        ...
        if args.mlp == True:
            ...
            self.v_cost = MLPAgent(args).to(self.device)
        else:
            self.critic = TransformerAgent(args).to(self.device)
            self.v_cost = TransformerAgent(args).to(self.device)
    ...
    def learn(self, single=False):
        # input dim [batch * context_len * dim], batch -> [num_envs * time_steps]
        state, action, reward, cost, done, next_state = self.memory.sample(self.batch_size)
        
        if single:
            print('Attention! Using single IFF channel')
            state = np.expand_dims(state[0,:,:,:], axis=0)
            next_state = np.expand_dims(next_state[0,:,:,:], axis=0)
            action = np.expand_dims(action[0,:,:], axis=0)
            reward = np.expand_dims(reward[0,:], axis=0)
            done = np.expand_dims(done[0,:], axis=0)
            print('state shape: ', state.shape)

        avg_cost = np.mean(cost)
        self.pid.pid_update(avg_cost)

        penal = self.pid.lagrangian_multiplier
        ...
        td_target_c = cost + self.gamma * self.v_cost.getValue(next_state).reshape(state.shape[0], state.shape[1]) * (1 - done) #20*1
        td_error_c = td_target_c - self.v_cost.getValue(state).reshape(state.shape[0], state.shape[1])
        advantage_c = compute_advantage(self.gamma, self.lamda, td_error_c.cpu()).to(self.device)
        # [trick] : advantage normalization
        td_lamda_target_c = advantage_c + self.v_cost.getValue(state).reshape(state.shape[0], state.shape[1])
        # advantage_c = ((advantage_c - advantage_c.mean()) / (advantage_c.std() +1e-5)).detach()

        safe_advantage = (advantage - penal * advantage_c) / (1 + penal)
        safe_advantage = ((safe_advantage - safe_advantage.mean()) / (safe_advantage.std() +1e-5)).detach()
        ...
        for _ in range(self.epoch):
            ...
            surprise = ratio * safe_advantage
            clipped_surprise = torch.clamp(ratio, 1 - self.clip, 1 + self.clip) * safe_advantage
            ...
            cost_loss = F.mse_loss(td_lamda_target_c.detach(), self.v_cost.getValue(state).reshape(state.shape[0], state.shape[1]))
            # update
            self.actor.optimizer.zero_grad()
            self.critic.optimizer.zero_grad()
            self.v_cost.optimizer.zero_grad()
            ...
            nn.utils.clip_grad_norm_(self.v_cost.parameters(), self.grad_norm
                                     )
            self.actor.optimizer.step()
            self.critic.optimizer.step()
            self.v_cost.optimizer.step()
        ...
        self.cost_loss = cost_loss.item()

        return actor_loss, critic_loss, cost_loss
    ...
    def save(self, save_path, episode):
        ...
        model_cost_path = os.path.join(base_path, "cost_" + str(episode) + '_' + time_str + ".pth")
        torch.save(self.actor.state_dict(), model_actor_path)
        torch.save(self.critic.state_dict(), model_critic_path)
        torch.save(self.v_cost.state_dict(), model_cost_path)
    ...
    def load_c(self, file):
        # load data from file, and map to the correct device
        # self.actor.load_state_dict(torch.load(file, map_location='cuda:0'))
        self.v_cost.load_state_dict(torch.load(file,  map_location=self.device), strict=False)
        self.v_cost.reset_optimizer()
