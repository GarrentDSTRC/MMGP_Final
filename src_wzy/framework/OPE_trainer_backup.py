...
class Transition(object):
    ...
    def compute_reward_done(self, state, pre_fys=None):
        if len(state.shape) == 3:
            Fx = state[:, :, 1]
            Fy = state[:, :, 2]
            # Clipped_Fx = np.clip(Fx, -1.0, 2.0)
            # Clipped_Fy = np.clip(Fy, -1.2, 2.0)
            # Clipped_Fx = self.symlog_clip(Fx, 8.0)
            Clipped_Fx = self.double_symlog_clip(Fx, 9.0, 16.0)
    ...
    def step(
        ...
        if re_flag==True:
            ...
            reward, done = self.compute_reward_done(next_obs, pre_fys=pre_fys)
            info = {}
