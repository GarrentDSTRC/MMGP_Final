...
class TPPO(object):
    ...
    def learn(self, single=False):
        ...
        if single:
            ...
            action = np.expand_dims(action[1,:,:], axis=0)
            reward = np.expand_dims(reward[1,:], axis=0)
            done = np.expand_dims(done[1,:], axis=0)
